...
/**
 * `Typography` с применённым масштабом шрифта из [LocalFontScale].
 *
 * Все размеры в `sp` (fontSize, lineHeight, letterSpacing) умножаются на scale,
 * потому что в этом проекте все текстовые стили используют только `sp`.
 * Ползунок «Размер шрифта» в настройках реально меняет размер текста во всём UI.
 */
...
private fun TextStyle.scaled(scale: Float): TextStyle = copy(
    fontSize = fontSize.value.sp * scale,
    lineHeight = lineHeight.value.sp * scale,
    letterSpacing = letterSpacing.value.sp * scale,
)